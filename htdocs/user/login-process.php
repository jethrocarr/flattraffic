...
if (user_online())
{
	...
	$_SESSION["error"]["username_flattraffic"] = "error";
	$_SESSION["error"]["password_flattraffic"] = "error";
...
else
{
	// check & convert input
	$instance	= NULL;
	$username	= security_form_input("/^[A-Za-z0-9.]*$/", "username_flattraffic", 4, "Please enter a username.");
	$password	= security_form_input("/^\S*$/", "password_flattraffic", 4, "Please enter a password.");
	...
	else
	{
		...
		if (!$_SESSION["error"]["message"])
		{
			...
			$_SESSION["error"]["username_flattraffic-error"] = 1;
			$_SESSION["error"]["password_flattraffic-error"] = 1;
		}

		if ($result == -3)
		{
			$_SESSION["error"]["instance_flattraffic-error"] = 1;
		}
